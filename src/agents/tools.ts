...
export const skillReader = tool(
  async ({ filePath }) => {
    try {
      ...
      return `Error reading skill file: ${errorMessage}`;
    }
  },
  {
    name: "skillReader",
 ...
 */
export const fileExplorer = tool(
  async ({ directoryPath }) => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      return `Error exploring directory: ${errorMessage}`;
    }
  },
  {
    name: "fileExplorer",
...
export const specLookup = tool(
  async ({ query }) => {
    ...
    `;
    return specDocs; // In a real app, this could be a vector store search
  },
  {
    name: "specLookup",
    description:
